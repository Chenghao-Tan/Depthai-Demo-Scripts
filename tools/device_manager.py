...
class SelectBootloader:
    def __init__(self, options, text):
        ...
        layout = [
            ...
            [
                sg.Text(
                    "Bootloader type:", font=("Arial", 10, "bold"), text_color="black"
                ),
        ...
        self.window = sg.Window(
            "Select bootloader", layout, size=(300, 150), modal=True, finalize=True
        )
...
class SelectIP:
    def __init__(self):
        ...
        layout = [
            [
                sg.Text(
                    "Specify the custom IP of the OAK PoE\ncamera you want to connect to"
                )
            ],
            [
        ...
        self.window = sg.Window(
            "Specify IP", layout, size=(300, 110), modal=True, finalize=True
        )
...
class SearchDevice:
    def __init__(self):
        ...
        layout = [
            [
                sg.Text(
                    "Select an OAK camera you would like to connect to.",
                    font=("Arial", 10, "bold"),
                )
            ],
            [
        ...
        self.window = sg.Window(
            "Select Device", layout, size=(550, 375), modal=True, finalize=True
        )
        self.search_devices()
    ...
    try:
        if devType == "POE":
            ...
            window.Element("networkTimeout").update(
                int(conf.getNetworkTimeout().total_seconds() * 1000)
            )
            ...
            window.Element("usbTimeout").update(
                int(conf.getUsbTimeout().total_seconds() * 1000)
            )
...
def flashBootloader(window, device: dai.DeviceInfo):
    ...
    try:
        sel = SelectBootloader(
            ["AUTO", "USB", "NETWORK"], "Select bootloader type to flash."
        )
        ok, type = sel.wait()
        if not ok:
    ...
    try:
        ...
        if devType == "POE":
            if staticIp:
                if (
                    check_ip(values["ip"])
                    and check_ip(values["mask"])
                    and check_ip(values["gateway"])
                ):
            ...
            if values["networkTimeout"] != "":
                if int(values["networkTimeout"]) >= 0:
                    conf.setNetworkTimeout(
                        timedelta(seconds=int(values["networkTimeout"]) / 1000)
                    )
                else:
...
def factoryReset(device: dai.DeviceInfo):
    sel = SelectBootloader(
        ["USB", "NETWORK"], "Select bootloader type used for factory reset."
    )
    ok, type = sel.wait()
    if not ok:
    ...
    try:
        ...
        msg = (
            ...
            if success
            else f"Factory reset failed. Error: {msg}"
        )
        pr.finish(msg)
        tmpBlFw.close()
    except Exception as ex:
...
aboutDeviceLayout = [
    [
        sg.Text(
            "About device", size=(30, 1), font=("Arial", 30, "bold"), text_color="black"
        )
    ],
    [sg.HSeparator()],
    [
    ...
    [
        ...
        sg.VSeparator(),
        sg.Text("-state-", key="devState", size=(30, 1)),
    ],
    [
        sg.Text(
            "Version of newest bootloader:",
            size=(30, 1),
            font=("Arial", 10, "bold"),
            text_color="black",
        ),
...
deviceConfigLayout = [
    ...
    [
        ...
        sg.Combo(
            USB_SPEEDS, "Select speed", key="usbSpeed", size=(30, 6), disabled=True
        ),
    ],
    [sg.HSeparator()],
    [
